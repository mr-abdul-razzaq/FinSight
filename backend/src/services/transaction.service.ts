...
export const scanReceiptService = async (
  ...
  if (subscription) {
    if (subscription.plan === "FREE" && subscription.usageCount.receiptScans >= 5) {
      throw new BadRequestException("You have reached the limit of 5 receipt scans for the FREE plan. Please upgrade to PRO.");
    }
    if (subscription.plan === "PRO" && subscription.usageCount.receiptScans >= 50) {
      throw new BadRequestException("You have reached the limit of 50 receipt scans for the PRO plan. Please upgrade to BUSINESS.");
    }
  }
  ...
  try {
    ...
    if (subscription) {
      subscription.usageCount.receiptScans += 1;
